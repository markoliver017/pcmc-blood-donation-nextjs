...
export const { handlers, signIn, signOut, auth } = NextAuth({
    ...
    callbacks: {
        async signIn({ user, account, profile, trigger }) {
            ...
            if (!admin?.roles?.length) return "/auth/error?error=No user role found";
            console.log("signin user roles", formatSeqObj(admin.roles));

            const active_roles = admin.roles.some(roles => roles.role.is_active == 1)
            console.log("signin isActive role", active_roles)
