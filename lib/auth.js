...
export const { handlers, signIn, signOut, auth } = NextAuth({
    providers: [
        ...
        CredentialsProvider({
            ...
            async authorize(credentials) {
                ...
                //     // Debug mode logic
                //     console.log(
                //         "Debug mode enabled with email:",
                //         credentials.debugEmail
                //     );
    ...
    callbacks: {
        async signIn({ user, account, profile, trigger }) {
            ...
            if (!admin) {
                console.log(
                    "SignIn Error: /auth/error?error=Email not registered!"
                );
                return `/auth/error?error=Email not registered!`;
            }
            // console.log("signin admin callback", admin)

            if (!admin?.roles?.length) {
                console.log(
                    "SignIn Error:/ auth / error ? error = No user role found"
                );
            ...
            if (!active_roles) {
                console.log(
                    "SignIn Error: /auth/error?error=No active role found"
                );
                return "/auth/error?error=No active role found";
            }
        ...
        async jwt({ token, account, trigger, user, session }) {
            ...
            if (trigger === "update") {
                ...
                if (session?.updated_roles)
                    token.updated_roles = session.updated_roles;
        ...
        async session({ session, token }) {
            ...
            if (token?.updated_roles) {
                session.user.roles = token?.updated_roles;
            }
