...
export const { handlers, signIn, signOut, auth } = NextAuth({
    ...
    callbacks: {
        async signIn({ user, account, profile, trigger }) {
            ...
            if (!admin) {
                console.log("SignIn Error: /auth/error?error=Email not registered!")
                return `/auth/error?error=Email not registered!`;
            }
            // console.log("signin admin callback", admin)

            if (!admin?.roles?.length) {
                console.log("SignIn Error:/ auth / error ? error = No user role found")
            ...
            if (!active_roles) {
                console.log("SignIn Error: /auth/error?error=No active role found")
