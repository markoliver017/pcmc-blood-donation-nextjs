const BloodRequestModel = (sequelize, DataTypes) => {
    const BloodRequest = sequelize.define(
        ...
        {
            hooks: {
                afterCreate: (bloodRequest, options) => {
                    const bloodRequestReferenceId = `BR${bloodRequest.id
                        .toString()
                        .padStart(6, "0")}`;
                    return bloodRequest.update(
                        {
                            blood_request_reference_id: bloodRequestReferenceId,
                        },
