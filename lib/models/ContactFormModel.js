const ContactFormModel = (sequelize, DataTypes) => {
    const ContactForm = sequelize.define(
        ...
        {
            hooks: {
                afterCreate: (contactForm, options) => {
                    const contactFormReferenceId = `CF${contactForm.id
                        .toString()
                        .padStart(6, "0")}`;
                    return contactForm.update(
                        {
                            contact_form_reference_id: contactFormReferenceId,
                        },
