...
export function logErrorToFile(error, context = "") {
    const msg = `❌ ERROR${context ? ` (${context})` : ""}: ${
        error.stack || error
    }`;
    writeLog(msg);
}
...
// const log = `[${new Date().toISOString()}] IP: ${ip} accessed ${url}\n`;

// // Append to log file (make sure this path exists and is writable)
// fs.appendFileSync(path.join(process.cwd(), "logs", "access.log"), log);
