                ...
                () =>
                    ({ chain, state }) => {
                        ...
                        const currentFontSize = textStyleMark?.attrs.fontSize
                            ...
                            : parseInt(this.options.defaultFontSize.replace('px', ''), 10);
                        const newFontSize = Math.min(currentFontSize + this.options.fontSizeStep, this.options.maxFontSize);
                        return chain()
                ...
                () =>
                    ({ chain, state }) => {
                        ...
                        const currentFontSize = textStyleMark?.attrs.fontSize
                            ? parseInt(textStyleMark.attrs.fontSize.replace('px', ''), 10)
                            : parseInt(this.options.defaultFontSize.replace('px', ''), 10);
                        const newFontSize = Math.max(currentFontSize - this.options.fontSizeStep, this.options.minFontSize);
                        return chain()
                            .setMark('textStyle', { fontSize: `${newFontSize}px` })
                            .run();
                    },
