...
import { Eye, Users, CheckIcon, XIcon, CalendarCog, MoreHorizontal, Command } from "lucide-react";
...
export default function AdminEventCard({ event, actionsType = "present", setIsLoading }) {
    // Helper for status badges
    const getRegStatusBadge = (status) => {
        if (status === "closed")
            return <div className="badge p-2 font-semibold text-xs badge-error"><GiClosedDoors /> Closed</div>;
        if (status === "completed")
            return <div className="badge p-2 font-semibold text-xs badge-success"><CheckIcon className="w-4 h-4" /> Completed</div>;
        if (status === "ongoing")
            return <div className="badge p-2 font-semibold text-xs badge-primary"><GiOpenBook /> Ongoing</div>;
        return <div className="badge p-2 font-semibold text-xs badge-warning"><ExclamationTriangleIcon /> {status}</div>;
    };
        ...
        <Card className="hover:ring-2 hover:ring-blue-400 group transition shadow-lg/40 max-h-max flex flex-col justify-between">
            <CardHeader>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                                ...
                                {actionsType === "present" && event.status === "approved" && event.registration_status === "ongoing" && (
                                    <DropdownMenuItem>
                                        <Link
                                ...
                                {event.status === "approved" && event.registration_status === "ongoing" && (
                                    <DropdownMenuItem>
                                        <Link
                                ...
                                {event.status === "approved" && event.registration_status !== "completed" && (
                                    <>
                                        <DropdownMenuItem>
                ...
                <CardDescription className="flex flex-col gap-1 mt-2">
                    <span className="text-sm text-gray-700 dark:text-gray-300 truncate">
                        {parse(event.description?.length > 80
            ...
            <CardContent className="flex flex-col gap-3">
                <div className="flex items-center gap-3 mb-2">
                    <CustomAvatar
                        avatar={event?.agency?.logo_url || event?.file_url || "/logo-1.jpeg"}
                ...
                <div className="flex flex-wrap gap-2 mb-2">
                    ...
                    {getRegStatusBadge(formatFormalName(event.registration_status))}
                    <span className="flex-items-center px-2 py-1 rounded bg-red-100 text-red-700 text-xs font-bold">
                        <Users className="w-4 h-4" />
                        {event.time_schedules?.reduce((acc, sched) => acc + sched?.donors?.length, 0) ?? 0}
                        {" "}donors
                    </span>
                </div>
